# TODO:
  # document better
  # tests
  # check unadjusted prices have correct shape
  # other backtesting function - results df should have lower case colnames


#' Equities Minimum Commission Backtest
#'
#' @description Quasi event-based simulation based on target share weights,
...
#' credit and debit cash balances. Conservative for large accounts; asymmetric
#' in reality (debit spread > credit spread).
...
#' }
#' @export
min_commission_backtest <- function(prices, unadjusted_prices, target_weights, interest_rates = NULL, short_borrow_costs = NULL, trade_buffer = 0., initial_cash = 10000, capitalise_profits = FALSE, include_initial_state = FALSE, commission_fun, ..., broker_interest_spread = 1.0/(100*365), maint_margin = 0.25) {
    ...
    if(equity > 0) {
      ...
      if(equity < MAINT_MARGIN*sum(abs(share_pos) * current_price, na.rm = TRUE)) {
        ...
        liq_shares <- -sign(share_pos) * pmin(
          ceiling(liquidate_factor * abs(share_pos)),  # ceiling to deal in integer number of contracts, liquidate at least 1 contract
          abs(share_pos)
      ...
      if(post_trade_equity < post_trade_margin) {
        ...
        reduce_shares <- -sign(target_shares) * pmin(
          trunc(reduce_by * abs(target_shares)),  # trunc to deal in integer number of contracts
          abs(target_shares)
        )
